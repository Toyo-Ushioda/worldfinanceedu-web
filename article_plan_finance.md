...
1. slug: npv-irr | status: done (published 2026-08-01)
   Title案: NPVとIRRの違いと使い分け|投資判断の2大指標をわかりやすく解説
   KW: NPV IRR 違い, NPV とは, IRR とは, 投資判断 指標
...
23. slug: earnout | status: pending
    Title案: アーンアウトとは|売り手と買い手の価格ギャップを埋める条件設計
    KW: アーンアウト とは, アーンアウト M&A, 条件付対価
    Angle: 実務。価格が折り合わないときの設計、KPI設定の落とし穴、日本での使われ方。内部リンク: ma-process, startup-valuation
    Cluster: M&A実務(実務)

24. slug: dcf-excel | status: pending
    Title案: DCF法をExcelで組む手順|モデルの構成と実務のチェックポイント
    KW: DCF エクセル 作り方, DCFモデル 作成, 企業価値 エクセル
    Angle: 実務。3表連動→FCF→WACC→TV→感応度表という構成、ハードコードを避ける等の実務作法。内部リンク: dcf-method, terminal-value, wacc-practice
    Cluster: 企業価値評価(実務)

25. slug: ai-finance-career | status: pending
